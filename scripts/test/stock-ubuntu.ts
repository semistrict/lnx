...
try {
  ...
  await testStep("fresh stock shape", async () => {
    const shape = await lnx(ctx, [
      ...
      "bash",
      "-lc",
      'cat /proc/1/comm; readlink /proc/1/root; findmnt -n -o PROPAGATION /; if test -e /newroot || test -e /oldroot; then echo leaked; else echo clean; fi',
    ]);
    assertEq(shape.stdout, "systemd\n/\nprivate\nclean", "stock root handoff");
  });

  await testStep("apt works", async () => {
    await lnx(ctx, ["apt-get", "update"], { timeoutMs: 300_000 });
  ...
  await testStep("snap hello-world works", async () => {
    await lnx(ctx, ["bash", "-lc", "DEBIAN_FRONTEND=noninteractive apt-get install -y snapd squashfs-tools"], { timeoutMs: 300_000 });
    await lnx(ctx, ["systemctl", "enable", "--now", "snapd.socket"]);
    await lnx(ctx, ["bash", "-lc", "systemctl start snapd.service || true"]);
    const version = await lnx(ctx, ["snap", "version"]);
    assertContains(version.stdout, "snapd", "snapd version");
    await lnx(ctx, ["snap", "install", "hello-world"], { timeoutMs: 600_000 });
