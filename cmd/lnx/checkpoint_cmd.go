...
func runCheckpointRestore(cmd *cobra.Command, args []string) error {
	...
	criuDir := filepath.Join(dir, "checkpoints", name)
	if _, err := os.Stat(filepath.Join(criuDir, "rootfs.ext4")); err == nil {
		return restoreCRIUCheckpoint(dir, criuDir, name)
	}

	// Fall back to disk-only checkpoint (.ext4 file).
	return restoreDiskCheckpoint(dir, name)
}

func restoreCRIUCheckpoint(instanceDir, criuDir, name string) error {
	rootfsPath := filepath.Join(instanceDir, "rootfs.ext4")
	criuPath := filepath.Join(instanceDir, "criu.ext4")
...
func restoreDiskCheckpoint(dir, name string) error {
	cpPath, err := resolveNamedCheckpoint(dir, name)
	if err != nil {
		return err
	}
